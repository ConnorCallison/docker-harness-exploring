import type { Route } from "./+types/content.navigation";
import { useState, useEffect, useCallback, useMemo } from "react";
import { cn } from "~/lib/cn";
import { formatDate, formatCurrency, formatNumber, formatRelative } from "~/lib/format";
import { z } from "zod";
import { RevenueChart } from "~/components/dashboard/RevenueChart";
import { MaintenanceBanner } from "~/components/features/MaintenanceBanner";
...
const ContentNavigationFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentNavigationFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 228;
  const items: ContentNavigationItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ContentNavigationActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ContentNavigationActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Content Navigation | Docker Harness App" },
    { name: "description", content: "Manage content navigation" },
  ];
}

// ─── Component ───

export default function ContentNavigation({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Navigation</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content navigation</p>
        </div>
