import type { Route } from "./+types/dashboard.metrics";
...
interface DashboardMetricsItem {
  id: string;
  name: string;
...
const DashboardMetricsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = DashboardMetricsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 246;
  const items: DashboardMetricsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const DashboardMetricsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = DashboardMetricsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Dashboard Metrics | Docker Harness App" },
    { name: "description", content: "Manage dashboard metrics" },
  ];
}

// ─── Component ───

export default function DashboardMetrics({ loaderData }: Route.ComponentProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track dashboard metrics</p>
        </div>
