...
const UsersNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UsersNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 208;
  const items: UsersNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const UsersNewActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = UsersNewActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Users New | Docker Harness App" },
    { name: "description", content: "Manage users new" },
  ];
}

// ─── Component ───

export default function UsersNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Users New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track users new</p>
        </div>
