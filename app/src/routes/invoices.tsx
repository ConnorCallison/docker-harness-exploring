...
const InvoicesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = InvoicesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 203;
  const items: InvoicesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const InvoicesActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = InvoicesActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Invoices | Docker Harness App" },
    { name: "description", content: "Manage invoices" },
  ];
}

// ─── Component ───

export default function Invoices({ children }: { children?: React.ReactNode }) {
