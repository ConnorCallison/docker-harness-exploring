...
const InvoiceNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = InvoiceNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 249;
  const items: InvoiceNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `invoice-${idx + 1}`,
      name: `Invoice Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for invoice item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const InvoiceNewActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = InvoiceNewActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Invoice New | Docker Harness App" },
    { name: "description", content: "Manage invoice new" },
  ];
}

// ─── Component ───

export default function InvoiceNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Invoice New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track invoice new</p>
        </div>
