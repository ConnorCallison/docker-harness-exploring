...
import { InlineEdit } from "~/components/data/InlineEdit";
...
const ProductNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 154;
  const items: ProductNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ProductNewActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ProductNewActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Product New | Docker Harness App" },
    { name: "description", content: "Manage product new" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product new</p>
        </div>
