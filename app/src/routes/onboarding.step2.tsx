import type { Route } from "./+types/onboarding.step2";
...
const OnboardingStep2FilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OnboardingStep2FilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 133;
  const items: OnboardingStep2Item[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Onboarding Step2 | Docker Harness App" },
    { name: "description", content: "Manage onboarding step2" },
  ];
}

// ─── Component ───

export default function OnboardingStep2({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Onboarding Step2</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track onboarding step2</p>
        </div>
