import type { Route } from "./+types/reports.sales";
...
const ReportsSalesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ReportsSalesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 153;
  const items: ReportsSalesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ReportsSalesActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ReportsSalesActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Reports Sales | Docker Harness App" },
    { name: "description", content: "Manage reports sales" },
  ];
}

// ─── Component ───

export default function ReportsSales({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Reports Sales</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track reports sales</p>
        </div>
