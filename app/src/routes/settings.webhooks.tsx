import type { Route } from "./+types/settings.webhooks";
...
interface SettingsWebhooksItem {
  id: string;
  name: string;
...
const SettingsWebhooksFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsWebhooksFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 104;
  const items: SettingsWebhooksItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Settings Webhooks | Docker Harness App" },
    { name: "description", content: "Manage settings webhooks" },
  ];
}

// ─── Component ───

export default function SettingsWebhooks({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Webhooks</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings webhooks</p>
        </div>
