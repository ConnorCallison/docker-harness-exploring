import type { Route } from "./+types/orders.shipments";
...
const OrderShipmentsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrderShipmentsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 109;
  const items: OrderShipmentsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Order Shipments | Docker Harness App" },
    { name: "description", content: "Manage order shipments" },
  ];
}

// ─── Component ───

export default function OrderShipments({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Order Shipments</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track order shipments</p>
        </div>
