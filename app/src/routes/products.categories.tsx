...
interface ProductCategoriesItem {
  id: string;
  name: string;
...
const ProductCategoriesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductCategoriesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 98;
  const items: ProductCategoriesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Product Categories | Docker Harness App" },
...
export default function ProductCategories({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Categories</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product categories</p>
        </div>
