...
const SupportTicketsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SupportTicketsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 219;
  const items: SupportTicketsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SupportTicketsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SupportTicketsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Support Tickets | Docker Harness App" },
...
export default function SupportTickets({ loaderData }: Route.ComponentProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track support tickets</p>
        </div>
