...
interface SettingsIntegrationsItem {
  id: string;
  name: string;
...
const SettingsIntegrationsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsIntegrationsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 221;
  const items: SettingsIntegrationsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsIntegrationsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsIntegrationsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Integrations | Docker Harness App" },
    { name: "description", content: "Manage settings integrations" },
  ];
}

// ─── Component ───

export default function SettingsIntegrations({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Integrations</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings integrations</p>
        </div>
