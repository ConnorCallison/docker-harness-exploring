...
const UserEditFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UserEditFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 183;
  const items: UserEditItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const UserEditActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = UserEditActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "User Edit | Docker Harness App" },
    { name: "description", content: "Manage user edit" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">User Edit</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track user edit</p>
        </div>
