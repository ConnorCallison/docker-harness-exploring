import type { Route } from "./+types/products.inventory";
...
const ProductInventoryFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductInventoryFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 103;
  const items: ProductInventoryItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Product Inventory | Docker Harness App" },
    { name: "description", content: "Manage product inventory" },
  ];
}

// ─── Component ───

export default function ProductInventory({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Inventory</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product inventory</p>
        </div>
