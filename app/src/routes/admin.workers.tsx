import type { Route } from "./+types/admin.workers";
...
const AdminWorkersFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = AdminWorkersFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 144;
  const items: AdminWorkersItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Admin Workers | Docker Harness App" },
    { name: "description", content: "Manage admin workers" },
  ];
}

// ─── Component ───

export default function AdminWorkers({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Admin Workers</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track admin workers</p>
        </div>
