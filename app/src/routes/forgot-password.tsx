...
const ForgotPasswordFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ForgotPasswordFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 148;
  const items: ForgotPasswordItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `forgot-${idx + 1}`,
      name: `Forgot Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for forgot item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
export function meta() {
  return [
    { title: "Forgot Password | Docker Harness App" },
    { name: "description", content: "Manage forgot password" },
  ];
}

// ─── Component ───

export default function ForgotPassword({ children }: { children?: React.ReactNode }) {
