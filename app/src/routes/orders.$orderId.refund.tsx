import type { Route } from "./+types/orders.$orderId.refund";
...
const OrderRefundFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrderRefundFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 241;
  const items: OrderRefundItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const OrderRefundActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = OrderRefundActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Order Refund | Docker Harness App" },
    { name: "description", content: "Manage order refund" },
  ];
}

// ─── Component ───

export default function OrderRefund({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Order Refund</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track order refund</p>
        </div>
