...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OnboardingFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 111;
  const items: OnboardingItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Onboarding | Docker Harness App" },
    { name: "description", content: "Manage onboarding" },
  ];
}
