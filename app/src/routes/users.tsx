...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UsersFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 242;
  const items: UsersItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const UsersActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = UsersActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Users | Docker Harness App" },
