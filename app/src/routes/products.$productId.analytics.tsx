import type { Route } from "./+types/products.$productId.analytics";
...
const ProductAnalyticsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductAnalyticsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 153;
  const items: ProductAnalyticsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ProductAnalyticsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ProductAnalyticsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Product Analytics | Docker Harness App" },
    { name: "description", content: "Manage product analytics" },
  ];
}

// ─── Component ───

export default function ProductAnalytics({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Analytics</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product analytics</p>
        </div>
