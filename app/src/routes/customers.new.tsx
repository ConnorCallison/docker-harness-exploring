...
const CustomerNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = CustomerNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 136;
  const items: CustomerNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Customer New | Docker Harness App" },
    { name: "description", content: "Manage customer new" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Customer New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track customer new</p>
        </div>
