...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ReportsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 216;
  const items: ReportsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ReportsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ReportsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Reports | Docker Harness App" },
    { name: "description", content: "Manage reports" },
  ];
}
