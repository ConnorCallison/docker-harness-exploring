import type { Route } from "./+types/marketing.analytics";
...
import { DragSortList } from "~/components/data/DragSortList";
import { CandlestickChart } from "~/components/charts/CandlestickChart";
import { ConditionEditor } from "~/components/forms/ConditionEditor";

// ─── Types ───

interface MarketingAnalyticsItem {
  id: string;
  name: string;
...
const MarketingAnalyticsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingAnalyticsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 210;
  const items: MarketingAnalyticsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const MarketingAnalyticsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = MarketingAnalyticsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Marketing Analytics | Docker Harness App" },
    { name: "description", content: "Manage marketing analytics" },
  ];
}

// ─── Component ───

export default function MarketingAnalytics({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Analytics</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing analytics</p>
        </div>
