...
const LoginFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = LoginFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 199;
  const items: LoginItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `login-${idx + 1}`,
      name: `Login Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for login item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const LoginActionSchema = z.discriminatedUnion("intent", [
...
export function meta() {
  return [
    { title: "Login | Docker Harness App" },
    { name: "description", content: "Manage login" },
  ];
}
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Login</h1>
          </div>
