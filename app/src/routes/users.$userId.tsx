...
const UserDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UserDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 114;
  const items: UserDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "User Detail | Docker Harness App" },
    { name: "description", content: "Manage user detail" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">User Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track user detail</p>
        </div>
