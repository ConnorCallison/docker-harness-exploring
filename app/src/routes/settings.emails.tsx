import type { Route } from "./+types/settings.emails";
...
const SettingsEmailsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsEmailsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 101;
  const items: SettingsEmailsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Settings Emails | Docker Harness App" },
    { name: "description", content: "Manage settings emails" },
  ];
}

// ─── Component ───

export default function SettingsEmails({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Emails</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings emails</p>
        </div>
