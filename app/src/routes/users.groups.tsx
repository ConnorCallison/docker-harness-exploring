import type { Route } from "./+types/users.groups";
...
const UsersGroupsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UsersGroupsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 238;
  const items: UsersGroupsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `users-${idx + 1}`,
      name: `Users Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for users item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const UsersGroupsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = UsersGroupsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Users Groups | Docker Harness App" },
    { name: "description", content: "Manage users groups" },
  ];
}

// ─── Component ───

export default function UsersGroups({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Users Groups</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track users groups</p>
        </div>
