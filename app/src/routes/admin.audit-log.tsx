...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = AdminAuditLogFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 222;
  const items: AdminAuditLogItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const AdminAuditLogActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = AdminAuditLogActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Admin Audit Log | Docker Harness App" },
    { name: "description", content: "Manage admin audit log" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Admin Audit Log</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track admin audit log</p>
        </div>
