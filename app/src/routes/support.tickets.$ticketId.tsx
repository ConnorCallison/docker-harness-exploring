import type { Route } from "./+types/support.tickets.$ticketId";
...
interface SupportTicketDetailItem {
  id: string;
  name: string;
...
const SupportTicketDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SupportTicketDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 217;
  const items: SupportTicketDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `support-${idx + 1}`,
      name: `Support Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for support item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const SupportTicketDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SupportTicketDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Support Ticket Detail | Docker Harness App" },
    { name: "description", content: "Manage support ticket detail" },
  ];
}

// ─── Component ───

export default function SupportTicketDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Support Ticket Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track support ticket detail</p>
        </div>
