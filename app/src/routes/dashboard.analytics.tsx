...
interface DashboardAnalyticsItem {
  id: string;
  name: string;
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = DashboardAnalyticsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 172;
  const items: DashboardAnalyticsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const DashboardAnalyticsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = DashboardAnalyticsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Dashboard Analytics | Docker Harness App" },
    { name: "description", content: "Manage dashboard analytics" },
  ];
}

// ─── Component ───

export default function DashboardAnalytics({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Dashboard Analytics</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track dashboard analytics</p>
        </div>
