import type { Route } from "./+types/support.knowledge-base.$articleId";
...
interface KnowledgeBaseArticleItem {
  id: string;
  name: string;
...
const KnowledgeBaseArticleFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = KnowledgeBaseArticleFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 158;
  const items: KnowledgeBaseArticleItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `knowledge-${idx + 1}`,
      name: `Knowledge Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for knowledge item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const KnowledgeBaseArticleActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = KnowledgeBaseArticleActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Knowledge Base Article | Docker Harness App" },
    { name: "description", content: "Manage knowledge base article" },
  ];
}

// ─── Component ───

export default function KnowledgeBaseArticle({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Knowledge Base Article</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track knowledge base article</p>
        </div>
