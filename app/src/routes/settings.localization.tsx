import type { Route } from "./+types/settings.localization";
...
interface SettingsLocalizationItem {
  id: string;
  name: string;
...
const SettingsLocalizationFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsLocalizationFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 97;
  const items: SettingsLocalizationItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Settings Localization | Docker Harness App" },
    { name: "description", content: "Manage settings localization" },
  ];
}

// ─── Component ───

export default function SettingsLocalization({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Localization</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings localization</p>
        </div>
