import type { Route } from "./+types/support.tickets.new";
...
interface SupportTicketNewItem {
  id: string;
  name: string;
...
const SupportTicketNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SupportTicketNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 86;
  const items: SupportTicketNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Support Ticket New | Docker Harness App" },
    { name: "description", content: "Manage support ticket new" },
  ];
}

// ─── Component ───

export default function SupportTicketNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Support Ticket New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track support ticket new</p>
        </div>
