import type { Route } from "./+types/onboarding.step3";
...
const OnboardingStep3FilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OnboardingStep3FilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 134;
  const items: OnboardingStep3Item[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Onboarding Step3 | Docker Harness App" },
    { name: "description", content: "Manage onboarding step3" },
  ];
}

// ─── Component ───

export default function OnboardingStep3({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Onboarding Step3</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track onboarding step3</p>
        </div>
