import type { Route } from "./+types/customers.$customerId.segments";
...
interface CustomerSegmentsItem {
  id: string;
  name: string;
...
const CustomerSegmentsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = CustomerSegmentsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 155;
  const items: CustomerSegmentsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `customer-${idx + 1}`,
      name: `Customer Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for customer item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const CustomerSegmentsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = CustomerSegmentsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Customer Segments | Docker Harness App" },
    { name: "description", content: "Manage customer segments" },
  ];
}

// ─── Component ───

export default function CustomerSegments({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Customer Segments</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track customer segments</p>
        </div>
