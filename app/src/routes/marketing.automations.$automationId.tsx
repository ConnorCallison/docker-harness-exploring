import type { Route } from "./+types/marketing.automations.$automationId";
...
interface MarketingAutomationDetailItem {
  id: string;
  name: string;
...
const MarketingAutomationDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingAutomationDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 88;
  const items: MarketingAutomationDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Marketing Automation Detail | Docker Harness App" },
    { name: "description", content: "Manage marketing automation detail" },
  ];
}

// ─── Component ───

export default function MarketingAutomationDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Automation Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing automation detail</p>
        </div>
