...
const OrdersListFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrdersListFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 215;
  const items: OrdersListItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `orders-${idx + 1}`,
      name: `Orders Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for orders item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const OrdersListActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = OrdersListActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Orders List | Docker Harness App" },
    { name: "description", content: "Manage orders list" },
  ];
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track orders list</p>
        </div>
