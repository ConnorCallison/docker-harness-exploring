...
interface DashboardSettingsItem {
  id: string;
  name: string;
...
const DashboardSettingsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = DashboardSettingsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 192;
  const items: DashboardSettingsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const DashboardSettingsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = DashboardSettingsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Dashboard Settings | Docker Harness App" },
    { name: "description", content: "Manage dashboard settings" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Dashboard Settings</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track dashboard settings</p>
        </div>
