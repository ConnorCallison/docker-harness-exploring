...
interface AdminFeatureFlagsItem {
  id: string;
  name: string;
...
const AdminFeatureFlagsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = AdminFeatureFlagsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 208;
  const items: AdminFeatureFlagsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const AdminFeatureFlagsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = AdminFeatureFlagsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Admin Feature Flags | Docker Harness App" },
    { name: "description", content: "Manage admin feature flags" },
  ];
}

// ─── Component ───

export default function AdminFeatureFlags({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Admin Feature Flags</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track admin feature flags</p>
        </div>
