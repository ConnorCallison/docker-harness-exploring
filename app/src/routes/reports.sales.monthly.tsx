import type { Route } from "./+types/reports.sales.monthly";
...
interface ReportsSalesMonthlyItem {
  id: string;
  name: string;
...
const ReportsSalesMonthlyFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ReportsSalesMonthlyFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 226;
  const items: ReportsSalesMonthlyItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ReportsSalesMonthlyActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ReportsSalesMonthlyActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Reports Sales Monthly | Docker Harness App" },
    { name: "description", content: "Manage reports sales monthly" },
  ];
}

// ─── Component ───

export default function ReportsSalesMonthly({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Reports Sales Monthly</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track reports sales monthly</p>
        </div>
