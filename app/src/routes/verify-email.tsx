...
const VerifyEmailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = VerifyEmailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 199;
  const items: VerifyEmailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `verify-${idx + 1}`,
      name: `Verify Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for verify item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const VerifyEmailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = VerifyEmailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Verify Email | Docker Harness App" },
    { name: "description", content: "Manage verify email" },
  ];
}

// ─── Component ───

export default function VerifyEmail({ children }: { children?: React.ReactNode }) {
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Verify Email</h1>
          </div>
