...
const InvoicesListFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = InvoicesListFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 169;
  const items: InvoicesListItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const InvoicesListActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = InvoicesListActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Invoices List | Docker Harness App" },
    { name: "description", content: "Manage invoices list" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Invoices List</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track invoices list</p>
        </div>
