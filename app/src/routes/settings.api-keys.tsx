...
const SettingsApiKeysFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsApiKeysFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 242;
  const items: SettingsApiKeysItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsApiKeysActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsApiKeysActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Api Keys | Docker Harness App" },
    { name: "description", content: "Manage settings api keys" },
  ];
}

// ─── Component ───

export default function SettingsApiKeys({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Api Keys</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings api keys</p>
        </div>
