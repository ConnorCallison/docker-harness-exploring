import type { Route } from "./+types/marketing.email.templates";
...
interface MarketingEmailTemplatesItem {
  id: string;
  name: string;
...
const MarketingEmailTemplatesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingEmailTemplatesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 239;
  const items: MarketingEmailTemplatesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const MarketingEmailTemplatesActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = MarketingEmailTemplatesActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Marketing Email Templates | Docker Harness App" },
    { name: "description", content: "Manage marketing email templates" },
  ];
}

// ─── Component ───

export default function MarketingEmailTemplates({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Email Templates</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing email templates</p>
        </div>
