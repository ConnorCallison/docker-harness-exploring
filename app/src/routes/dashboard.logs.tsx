...
const DashboardLogsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = DashboardLogsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 173;
  const items: DashboardLogsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const DashboardLogsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = DashboardLogsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Dashboard Logs | Docker Harness App" },
    { name: "description", content: "Manage dashboard logs" },
  ];
}

// ─── Component ───

export default function DashboardLogs({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Dashboard Logs</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track dashboard logs</p>
        </div>
