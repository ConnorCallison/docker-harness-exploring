import type { Route } from "./+types/onboarding.step1";
...
const OnboardingStep1FilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OnboardingStep1FilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 133;
  const items: OnboardingStep1Item[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `onboarding-${idx + 1}`,
      name: `Onboarding Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for onboarding item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
export function meta() {
  return [
    { title: "Onboarding Step1 | Docker Harness App" },
    { name: "description", content: "Manage onboarding step1" },
  ];
}

// ─── Component ───

export default function OnboardingStep1({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Onboarding Step1</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track onboarding step1</p>
        </div>
