import type { Route } from "./+types/content.media";
...
import { ThroughputWidget } from "~/components/dashboard/ThroughputWidget";
import { Label } from "~/components/ui/Label";
import { ThreeColumnLayout } from "~/components/layout/ThreeColumnLayout";
import { WordCloud } from "~/components/data/WordCloud";
...
const ContentMediaFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentMediaFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 220;
  const items: ContentMediaItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ContentMediaActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ContentMediaActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Content Media | Docker Harness App" },
    { name: "description", content: "Manage content media" },
  ];
}

// ─── Component ───

export default function ContentMedia({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Media</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content media</p>
        </div>
