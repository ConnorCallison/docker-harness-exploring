import type { Route } from "./+types/users.roles.$roleId";
...
const UsersRoleDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UsersRoleDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 142;
  const items: UsersRoleDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Users Role Detail | Docker Harness App" },
    { name: "description", content: "Manage users role detail" },
  ];
}

// ─── Component ───

export default function UsersRoleDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Users Role Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track users role detail</p>
        </div>
