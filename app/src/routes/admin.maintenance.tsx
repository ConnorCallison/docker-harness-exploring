...
const AdminMaintenanceFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = AdminMaintenanceFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 188;
  const items: AdminMaintenanceItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const AdminMaintenanceActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = AdminMaintenanceActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Admin Maintenance | Docker Harness App" },
    { name: "description", content: "Manage admin maintenance" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Admin Maintenance</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track admin maintenance</p>
        </div>
