import type { Route } from "./+types/admin.cache";
...
const AdminCacheFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = AdminCacheFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 197;
  const items: AdminCacheItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const AdminCacheActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = AdminCacheActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Admin Cache | Docker Harness App" },
    { name: "description", content: "Manage admin cache" },
  ];
}

// ─── Component ───

export default function AdminCache({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Admin Cache</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track admin cache</p>
        </div>
