import type { Route } from "./+types/support.knowledge-base";
...
const KnowledgeBaseFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = KnowledgeBaseFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 203;
  const items: KnowledgeBaseItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const KnowledgeBaseActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = KnowledgeBaseActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Knowledge Base | Docker Harness App" },
    { name: "description", content: "Manage knowledge base" },
  ];
}

// ─── Component ───

export default function KnowledgeBase({ loaderData }: Route.ComponentProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track knowledge base</p>
        </div>
