import type { Route } from "./+types/content.blog.$postId";
...
interface ContentBlogDetailItem {
  id: string;
  name: string;
...
const ContentBlogDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentBlogDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 209;
  const items: ContentBlogDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ContentBlogDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ContentBlogDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Content Blog Detail | Docker Harness App" },
    { name: "description", content: "Manage content blog detail" },
  ];
}

// ─── Component ───

export default function ContentBlogDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Blog Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content blog detail</p>
        </div>
