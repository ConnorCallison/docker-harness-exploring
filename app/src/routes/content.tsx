...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 99;
  const items: ContentItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Content | Docker Harness App" },
    { name: "description", content: "Manage content" },
  ];
}
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Content</h1>
          </div>
