import type { Route } from "./+types/reports.sales.daily";
import { useState, useEffect, useCallback, useMemo } from "react";
import { cn } from "~/lib/cn";
import { formatDate, formatCurrency, formatNumber, formatRelative } from "~/lib/format";
import { z } from "zod";
import { SupportTicketList } from "~/components/dashboard/SupportTicketList";
import { KanbanBoard } from "~/components/features/KanbanBoard";
import { SelectField } from "~/components/forms/SelectField";
import { ViolinChart } from "~/components/charts/ViolinChart";
import { CellRenderer } from "~/components/data/CellRenderer";
import { Flex } from "~/components/layout/Flex";

// ─── Types ───

interface ReportsSalesDailyItem {
  id: string;
  name: string;
...
const ReportsSalesDailyFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ReportsSalesDailyFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 218;
  const items: ReportsSalesDailyItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `reports-${idx + 1}`,
      name: `Reports Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for reports item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const ReportsSalesDailyActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ReportsSalesDailyActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Reports Sales Daily | Docker Harness App" },
    { name: "description", content: "Manage reports sales daily" },
  ];
}

// ─── Component ───

export default function ReportsSalesDaily({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Reports Sales Daily</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track reports sales daily</p>
        </div>
