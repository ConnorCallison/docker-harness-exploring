import type { Route } from "./+types/content.pages.$pageId";
...
import { StatDisplay } from "~/components/data/StatDisplay";

// ─── Types ───

interface ContentPageDetailItem {
  id: string;
  name: string;
...
const ContentPageDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentPageDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 205;
  const items: ContentPageDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `content-${idx + 1}`,
      name: `Content Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for content item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const ContentPageDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ContentPageDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Content Page Detail | Docker Harness App" },
    { name: "description", content: "Manage content page detail" },
  ];
}

// ─── Component ───

export default function ContentPageDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Page Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content page detail</p>
        </div>
