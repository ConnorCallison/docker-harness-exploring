import type { Route } from "./+types/reports.custom.new";
...
interface ReportsCustomNewItem {
  id: string;
  name: string;
...
const ReportsCustomNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ReportsCustomNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 154;
  const items: ReportsCustomNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ReportsCustomNewActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ReportsCustomNewActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Reports Custom New | Docker Harness App" },
    { name: "description", content: "Manage reports custom new" },
  ];
}

// ─── Component ───

export default function ReportsCustomNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Reports Custom New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track reports custom new</p>
        </div>
