import type { Route } from "./+types/reports.exports";
...
import { LocaleConfigForm } from "~/components/forms/LocaleConfigForm";
...
const ReportsExportsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ReportsExportsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 227;
  const items: ReportsExportsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ReportsExportsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ReportsExportsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Reports Exports | Docker Harness App" },
    { name: "description", content: "Manage reports exports" },
  ];
}

// ─── Component ───

export default function ReportsExports({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Reports Exports</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track reports exports</p>
        </div>
