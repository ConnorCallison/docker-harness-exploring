...
interface SettingsSecurityItem {
  id: string;
  name: string;
...
const SettingsSecurityFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsSecurityFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 188;
  const items: SettingsSecurityItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsSecurityActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsSecurityActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Security | Docker Harness App" },
    { name: "description", content: "Manage settings security" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Security</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings security</p>
        </div>
