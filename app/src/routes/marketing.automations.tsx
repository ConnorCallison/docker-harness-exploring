import type { Route } from "./+types/marketing.automations";
...
interface MarketingAutomationsItem {
  id: string;
  name: string;
...
const MarketingAutomationsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingAutomationsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 118;
  const items: MarketingAutomationsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Marketing Automations | Docker Harness App" },
    { name: "description", content: "Manage marketing automations" },
  ];
}

// ─── Component ───

export default function MarketingAutomations({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Automations</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing automations</p>
        </div>
