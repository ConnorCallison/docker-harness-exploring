import type { Route } from "./+types/settings.security.two-factor";
import { useState, useEffect, useCallback, useMemo } from "react";
import { cn } from "~/lib/cn";
import { formatDate, formatCurrency, formatNumber, formatRelative } from "~/lib/format";
import { z } from "zod";
import { MetricsExplorer } from "~/components/dashboard/MetricsExplorer";
import { Table } from "~/components/ui/Table";
import { FloatingNav } from "~/components/layout/FloatingNav";

// ─── Types ───

interface SettingsTwoFactorItem {
  id: string;
  name: string;
...
const SettingsTwoFactorFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsTwoFactorFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 96;
  const items: SettingsTwoFactorItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Settings Two Factor | Docker Harness App" },
    { name: "description", content: "Manage settings two factor" },
  ];
}

// ─── Component ───

export default function SettingsTwoFactor({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Two Factor</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings two factor</p>
        </div>
