import type { Route } from "./+types/settings.billing.invoices";
...
import { P99LatencyWidget } from "~/components/dashboard/P99LatencyWidget";
import { ClipboardManager } from "~/components/features/ClipboardManager";
import { SwitchField } from "~/components/forms/SwitchField";

// ─── Types ───

interface SettingsBillingInvoicesItem {
  id: string;
  name: string;
...
const SettingsBillingInvoicesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsBillingInvoicesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 224;
  const items: SettingsBillingInvoicesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `settings-${idx + 1}`,
      name: `Settings Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for settings item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const SettingsBillingInvoicesActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsBillingInvoicesActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Billing Invoices | Docker Harness App" },
    { name: "description", content: "Manage settings billing invoices" },
  ];
}

// ─── Component ───

export default function SettingsBillingInvoices({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Billing Invoices</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings billing invoices</p>
        </div>
