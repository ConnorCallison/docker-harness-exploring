...
const OrderTrackingFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrderTrackingFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 185;
  const items: OrderTrackingItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const OrderTrackingActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = OrderTrackingActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Order Tracking | Docker Harness App" },
    { name: "description", content: "Manage order tracking" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Order Tracking</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track order tracking</p>
        </div>
