import type { Route } from "./+types/products.categories.$catId";
...
interface ProductCategoryDetailItem {
  id: string;
  name: string;
...
const ProductCategoryDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductCategoryDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 160;
  const items: ProductCategoryDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ProductCategoryDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ProductCategoryDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Product Category Detail | Docker Harness App" },
    { name: "description", content: "Manage product category detail" },
  ];
}

// ─── Component ───

export default function ProductCategoryDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Category Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product category detail</p>
        </div>
