import type { Route } from "./+types/users.$userId.sessions";
...
const UserSessionsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UserSessionsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 227;
  const items: UserSessionsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const UserSessionsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = UserSessionsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "User Sessions | Docker Harness App" },
    { name: "description", content: "Manage user sessions" },
  ];
}

// ─── Component ───

export default function UserSessions({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">User Sessions</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track user sessions</p>
        </div>
