import type { Route } from "./+types/settings.shipping";
...
interface SettingsShippingItem {
  id: string;
  name: string;
...
const SettingsShippingFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsShippingFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 170;
  const items: SettingsShippingItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsShippingActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsShippingActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Shipping | Docker Harness App" },
    { name: "description", content: "Manage settings shipping" },
  ];
}

// ─── Component ───

export default function SettingsShipping({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Shipping</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings shipping</p>
        </div>
