...
const CustomerEditFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = CustomerEditFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 102;
  const items: CustomerEditItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Customer Edit | Docker Harness App" },
    { name: "description", content: "Manage customer edit" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Customer Edit</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track customer edit</p>
        </div>
