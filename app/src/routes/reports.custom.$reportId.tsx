import type { Route } from "./+types/reports.custom.$reportId";
...
import { EmbedDialog } from "~/components/features/EmbedDialog";
import { RevenueChart } from "~/components/dashboard/RevenueChart";
import { DataList } from "~/components/ui/DataList";

// ─── Types ───

interface ReportsCustomDetailItem {
  id: string;
  name: string;
...
const ReportsCustomDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ReportsCustomDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 202;
  const items: ReportsCustomDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ReportsCustomDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ReportsCustomDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Reports Custom Detail | Docker Harness App" },
    { name: "description", content: "Manage reports custom detail" },
  ];
}

// ─── Component ───

export default function ReportsCustomDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Reports Custom Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track reports custom detail</p>
        </div>
