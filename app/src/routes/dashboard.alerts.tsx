...
const DashboardAlertsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = DashboardAlertsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 108;
  const items: DashboardAlertsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Dashboard Alerts | Docker Harness App" },
    { name: "description", content: "Manage dashboard alerts" },
  ];
}

// ─── Component ───

export default function DashboardAlerts({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Dashboard Alerts</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track dashboard alerts</p>
        </div>
