import type { Route } from "./+types/marketing.seo";
...
const MarketingSEOFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingSEOFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 187;
  const items: MarketingSEOItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const MarketingSEOActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = MarketingSEOActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Marketing S E O | Docker Harness App" },
    { name: "description", content: "Manage marketing s e o" },
  ];
}

// ─── Component ───

export default function MarketingSEO({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing S E O</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing s e o</p>
        </div>
