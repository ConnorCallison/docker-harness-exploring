import type { Route } from "./+types/settings.billing.payment-methods";
...
interface SettingsPaymentMethodsItem {
  id: string;
  name: string;
...
const SettingsPaymentMethodsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsPaymentMethodsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 197;
  const items: SettingsPaymentMethodsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsPaymentMethodsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsPaymentMethodsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Payment Methods | Docker Harness App" },
    { name: "description", content: "Manage settings payment methods" },
  ];
}

// ─── Component ───

export default function SettingsPaymentMethods({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Payment Methods</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings payment methods</p>
        </div>
