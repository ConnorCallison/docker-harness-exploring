...
const AdminSystemFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = AdminSystemFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 168;
  const items: AdminSystemItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const AdminSystemActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = AdminSystemActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Admin System | Docker Harness App" },
    { name: "description", content: "Manage admin system" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Admin System</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track admin system</p>
        </div>
