import type { Route } from "./+types/products.$productId.reviews";
...
const ProductReviewsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductReviewsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 137;
  const items: ProductReviewsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Product Reviews | Docker Harness App" },
    { name: "description", content: "Manage product reviews" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Reviews</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product reviews</p>
        </div>
