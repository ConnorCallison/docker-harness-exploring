import type { Route } from "./+types/marketing.campaigns.$campaignId";
...
import { ColumnDrag } from "~/components/data/ColumnDrag";

// ─── Types ───

interface MarketingCampaignDetailItem {
  id: string;
  name: string;
...
const MarketingCampaignDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingCampaignDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 217;
  const items: MarketingCampaignDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `marketing-${idx + 1}`,
      name: `Marketing Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for marketing item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const MarketingCampaignDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = MarketingCampaignDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Marketing Campaign Detail | Docker Harness App" },
    { name: "description", content: "Manage marketing campaign detail" },
  ];
}

// ─── Component ───

export default function MarketingCampaignDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Campaign Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing campaign detail</p>
        </div>
