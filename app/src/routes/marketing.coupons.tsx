...
interface MarketingCouponsItem {
  id: string;
  name: string;
...
const MarketingCouponsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingCouponsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 211;
  const items: MarketingCouponsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const MarketingCouponsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = MarketingCouponsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Marketing Coupons | Docker Harness App" },
    { name: "description", content: "Manage marketing coupons" },
  ];
}

// ─── Component ───

export default function MarketingCoupons({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Coupons</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing coupons</p>
        </div>
