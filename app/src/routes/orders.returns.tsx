import type { Route } from "./+types/orders.returns";
...
const OrderReturnsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrderReturnsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 228;
  const items: OrderReturnsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `order-${idx + 1}`,
      name: `Order Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for order item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const OrderReturnsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = OrderReturnsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Order Returns | Docker Harness App" },
    { name: "description", content: "Manage order returns" },
  ];
}

// ─── Component ───

export default function OrderReturns({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Order Returns</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track order returns</p>
        </div>
