...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UsersListFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 182;
  const items: UsersListItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const UsersListActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = UsersListActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Users List | Docker Harness App" },
    { name: "description", content: "Manage users list" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Users List</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track users list</p>
        </div>
