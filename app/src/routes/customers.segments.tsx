import type { Route } from "./+types/customers.segments";
...
const SegmentsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SegmentsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 242;
  const items: SegmentsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `segments-${idx + 1}`,
      name: `Segments Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for segments item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const SegmentsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SegmentsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Segments | Docker Harness App" },
    { name: "description", content: "Manage segments" },
  ];
}

// ─── Component ───

export default function Segments({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Segments</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track segments</p>
        </div>
