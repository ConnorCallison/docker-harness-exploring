...
const SettingsAccountFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsAccountFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 127;
  const items: SettingsAccountItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Settings Account | Docker Harness App" },
    { name: "description", content: "Manage settings account" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Account</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings account</p>
        </div>
