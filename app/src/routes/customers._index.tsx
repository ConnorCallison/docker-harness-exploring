...
const CustomersListFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = CustomersListFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 156;
  const items: CustomersListItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `customers-${idx + 1}`,
      name: `Customers Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for customers item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const CustomersListActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = CustomersListActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Customers List | Docker Harness App" },
    { name: "description", content: "Manage customers list" },
  ];
}

// ─── Component ───

export default function CustomersList({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Customers List</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track customers list</p>
        </div>
