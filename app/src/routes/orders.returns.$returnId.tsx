import type { Route } from "./+types/orders.returns.$returnId";
...
interface OrderReturnDetailItem {
  id: string;
  name: string;
...
const OrderReturnDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrderReturnDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 88;
  const items: OrderReturnDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Order Return Detail | Docker Harness App" },
    { name: "description", content: "Manage order return detail" },
  ];
}

// ─── Component ───

export default function OrderReturnDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Order Return Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track order return detail</p>
        </div>
