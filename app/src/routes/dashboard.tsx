...
import { WizardDialog } from "~/components/features/WizardDialog";
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = DashboardFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 126;
  const items: DashboardItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Dashboard | Docker Harness App" },
    { name: "description", content: "Manage dashboard" },
  ];
}
