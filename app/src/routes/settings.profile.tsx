...
const SettingsProfileFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsProfileFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 150;
  const items: SettingsProfileItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsProfileActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsProfileActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Profile | Docker Harness App" },
    { name: "description", content: "Manage settings profile" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Profile</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings profile</p>
        </div>
