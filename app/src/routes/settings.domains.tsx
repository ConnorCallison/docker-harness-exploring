...
const SettingsDomainsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsDomainsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 89;
  const items: SettingsDomainsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Settings Domains | Docker Harness App" },
    { name: "description", content: "Manage settings domains" },
  ];
}

// ─── Component ───

export default function SettingsDomains({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Domains</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings domains</p>
        </div>
