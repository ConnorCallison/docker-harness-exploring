...
interface SettingsTeamMemberItem {
  id: string;
  name: string;
...
const SettingsTeamMemberFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsTeamMemberFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 156;
  const items: SettingsTeamMemberItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsTeamMemberActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsTeamMemberActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Team Member | Docker Harness App" },
    { name: "description", content: "Manage settings team member" },
  ];
}

// ─── Component ───

export default function SettingsTeamMember({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Team Member</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings team member</p>
        </div>
