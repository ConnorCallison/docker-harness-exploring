import type { Route } from "./+types/settings.checkout";
...
interface SettingsCheckoutItem {
  id: string;
  name: string;
...
const SettingsCheckoutFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsCheckoutFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 115;
  const items: SettingsCheckoutItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Settings Checkout | Docker Harness App" },
    { name: "description", content: "Manage settings checkout" },
  ];
}

// ─── Component ───

export default function SettingsCheckout({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Checkout</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings checkout</p>
        </div>
