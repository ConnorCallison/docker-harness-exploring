...
const InvoiceDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = InvoiceDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 102;
  const items: InvoiceDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Invoice Detail | Docker Harness App" },
    { name: "description", content: "Manage invoice detail" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Invoice Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track invoice detail</p>
        </div>
