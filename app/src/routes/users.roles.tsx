import type { Route } from "./+types/users.roles";
...
const UsersRolesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UsersRolesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 124;
  const items: UsersRolesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Users Roles | Docker Harness App" },
    { name: "description", content: "Manage users roles" },
  ];
}

// ─── Component ───

export default function UsersRoles({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Users Roles</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track users roles</p>
        </div>
