import type { Route } from "./+types/customers.import";
...
const CustomerImportFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = CustomerImportFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 236;
  const items: CustomerImportItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const CustomerImportActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = CustomerImportActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Customer Import | Docker Harness App" },
    { name: "description", content: "Manage customer import" },
  ];
}

// ─── Component ───

export default function CustomerImport({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Customer Import</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track customer import</p>
        </div>
