...
const OrderDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrderDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 160;
  const items: OrderDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const OrderDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = OrderDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Order Detail | Docker Harness App" },
    { name: "description", content: "Manage order detail" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Order Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track order detail</p>
        </div>
