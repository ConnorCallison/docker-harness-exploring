import type { Route } from "./+types/marketing.campaigns.new";
...
interface MarketingCampaignNewItem {
  id: string;
  name: string;
...
const MarketingCampaignNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingCampaignNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 96;
  const items: MarketingCampaignNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Marketing Campaign New | Docker Harness App" },
    { name: "description", content: "Manage marketing campaign new" },
  ];
}

// ─── Component ───

export default function MarketingCampaignNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Campaign New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing campaign new</p>
        </div>
