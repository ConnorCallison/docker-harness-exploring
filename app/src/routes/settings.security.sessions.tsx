...
import { NotificationPrefsForm } from "~/components/forms/NotificationPrefsForm";
...
interface SettingsSessionsItem {
  id: string;
  name: string;
...
const SettingsSessionsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsSessionsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 154;
  const items: SettingsSessionsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsSessionsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsSessionsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Sessions | Docker Harness App" },
    { name: "description", content: "Manage settings sessions" },
  ];
}

// ─── Component ───

export default function SettingsSessions({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Sessions</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings sessions</p>
        </div>
