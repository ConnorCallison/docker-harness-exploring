...
import { NpsWidget } from "~/components/features/NpsWidget";
import { FormLabel } from "~/components/forms/FormLabel";

// ─── Types ───

interface DashboardReportsItem {
  id: string;
  name: string;
...
const DashboardReportsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = DashboardReportsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 212;
  const items: DashboardReportsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `dashboard-${idx + 1}`,
      name: `Dashboard Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for dashboard item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const DashboardReportsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = DashboardReportsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Dashboard Reports | Docker Harness App" },
    { name: "description", content: "Manage dashboard reports" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Dashboard Reports</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track dashboard reports</p>
        </div>
