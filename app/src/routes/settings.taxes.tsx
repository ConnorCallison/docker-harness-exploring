import type { Route } from "./+types/settings.taxes";
...
const SettingsTaxesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsTaxesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 185;
  const items: SettingsTaxesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsTaxesActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsTaxesActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Taxes | Docker Harness App" },
    { name: "description", content: "Manage settings taxes" },
  ];
}

// ─── Component ───

export default function SettingsTaxes({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Taxes</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings taxes</p>
        </div>
