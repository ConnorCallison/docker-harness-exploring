import type { Route } from "./+types/content.redirects";
...
interface ContentRedirectsItem {
  id: string;
  name: string;
...
const ContentRedirectsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentRedirectsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 148;
  const items: ContentRedirectsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Content Redirects | Docker Harness App" },
    { name: "description", content: "Manage content redirects" },
  ];
}

// ─── Component ───

export default function ContentRedirects({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Redirects</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content redirects</p>
        </div>
