import type { Route } from "./+types/content.blog";
...
const ContentBlogFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentBlogFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 138;
  const items: ContentBlogItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Content Blog | Docker Harness App" },
    { name: "description", content: "Manage content blog" },
  ];
}

// ─── Component ───

export default function ContentBlog({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Blog</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content blog</p>
        </div>
