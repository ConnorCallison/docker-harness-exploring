import type { Route } from "./+types/invoices.$invoiceId.send";
...
const InvoiceSendFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = InvoiceSendFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 134;
  const items: InvoiceSendItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Invoice Send | Docker Harness App" },
    { name: "description", content: "Manage invoice send" },
  ];
}

// ─── Component ───

export default function InvoiceSend({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Invoice Send</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track invoice send</p>
        </div>
