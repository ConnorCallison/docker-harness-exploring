import type { Route } from "./+types/invoices.templates";
...
interface InvoiceTemplatesItem {
  id: string;
  name: string;
...
const InvoiceTemplatesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = InvoiceTemplatesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 134;
  const items: InvoiceTemplatesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Invoice Templates | Docker Harness App" },
    { name: "description", content: "Manage invoice templates" },
  ];
}

// ─── Component ───

export default function InvoiceTemplates({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Invoice Templates</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track invoice templates</p>
        </div>
