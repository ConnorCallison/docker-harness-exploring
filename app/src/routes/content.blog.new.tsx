import type { Route } from "./+types/content.blog.new";
...
const ContentBlogNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentBlogNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 236;
  const items: ContentBlogNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ContentBlogNewActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ContentBlogNewActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Content Blog New | Docker Harness App" },
    { name: "description", content: "Manage content blog new" },
  ];
}

// ─── Component ───

export default function ContentBlogNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Blog New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content blog new</p>
        </div>
