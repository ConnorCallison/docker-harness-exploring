import type { Route } from "./+types/products.collections";
import { useState, useEffect, useCallback, useMemo } from "react";
import { cn } from "~/lib/cn";
import { formatDate, formatCurrency, formatNumber, formatRelative } from "~/lib/format";
import { z } from "zod";
import { CostExplorer } from "~/components/dashboard/CostExplorer";
...
interface ProductCollectionsItem {
  id: string;
  name: string;
...
const ProductCollectionsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductCollectionsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 225;
  const items: ProductCollectionsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    ...
    return {
      ...
      description: `Description for product item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const ProductCollectionsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ProductCollectionsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Product Collections | Docker Harness App" },
    { name: "description", content: "Manage product collections" },
  ];
}

// ─── Component ───

export default function ProductCollections({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Collections</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product collections</p>
        </div>
