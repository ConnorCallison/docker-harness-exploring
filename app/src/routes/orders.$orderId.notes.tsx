import type { Route } from "./+types/orders.$orderId.notes";
...
const OrderNotesFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = OrderNotesFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 124;
  const items: OrderNotesItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Order Notes | Docker Harness App" },
    { name: "description", content: "Manage order notes" },
  ];
}

// ─── Component ───

export default function OrderNotes({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Order Notes</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track order notes</p>
        </div>
