import type { Route } from "./+types/content.pages.new";
...
import { SelectableList } from "~/components/data/SelectableList";

// ─── Types ───

interface ContentPageNewItem {
  id: string;
  name: string;
...
const ContentPageNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ContentPageNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 83;
  const items: ContentPageNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Content Page New | Docker Harness App" },
    { name: "description", content: "Manage content page new" },
  ];
}

// ─── Component ───

export default function ContentPageNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Content Page New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track content page new</p>
        </div>
