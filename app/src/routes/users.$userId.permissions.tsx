import type { Route } from "./+types/users.$userId.permissions";
...
const UserPermissionsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UserPermissionsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 103;
  const items: UserPermissionsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "User Permissions | Docker Harness App" },
...
export default function UserPermissions({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">User Permissions</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track user permissions</p>
        </div>
