...
const SettingsTeamFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsTeamFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 168;
  const items: SettingsTeamItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsTeamActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsTeamActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Team | Docker Harness App" },
    { name: "description", content: "Manage settings team" },
  ];
}

// ─── Component ───

export default function SettingsTeam({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Team</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings team</p>
        </div>
