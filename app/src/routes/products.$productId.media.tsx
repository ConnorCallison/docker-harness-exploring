import type { Route } from "./+types/products.$productId.media";
...
const ProductMediaFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductMediaFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 236;
  const items: ProductMediaItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ProductMediaActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ProductMediaActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Product Media | Docker Harness App" },
    { name: "description", content: "Manage product media" },
  ];
}

// ─── Component ───

export default function ProductMedia({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Media</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product media</p>
        </div>
