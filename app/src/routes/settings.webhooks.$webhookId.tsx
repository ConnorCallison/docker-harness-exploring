import type { Route } from "./+types/settings.webhooks.$webhookId";
...
import { ChangelogViewer } from "~/components/features/ChangelogViewer";
...
interface SettingsWebhookDetailItem {
  id: string;
  name: string;
...
const SettingsWebhookDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = SettingsWebhookDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 233;
  const items: SettingsWebhookDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const SettingsWebhookDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = SettingsWebhookDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Settings Webhook Detail | Docker Harness App" },
    { name: "description", content: "Manage settings webhook detail" },
  ];
}

// ─── Component ───

export default function SettingsWebhookDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Settings Webhook Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track settings webhook detail</p>
        </div>
