import type { Route } from "./+types/marketing.coupons.new";
...
interface MarketingCouponNewItem {
  id: string;
  name: string;
...
const MarketingCouponNewFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = MarketingCouponNewFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 132;
  const items: MarketingCouponNewItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
export function meta() {
  return [
    { title: "Marketing Coupon New | Docker Harness App" },
    { name: "description", content: "Manage marketing coupon new" },
  ];
}

// ─── Component ───

export default function MarketingCouponNew({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Marketing Coupon New</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track marketing coupon new</p>
        </div>
