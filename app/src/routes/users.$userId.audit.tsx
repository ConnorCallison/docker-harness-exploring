import type { Route } from "./+types/users.$userId.audit";
...
const UserAuditFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = UserAuditFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 235;
  const items: UserAuditItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const UserAuditActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = UserAuditActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "User Audit | Docker Harness App" },
    { name: "description", content: "Manage user audit" },
  ];
}

// ─── Component ───

export default function UserAudit({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">User Audit</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track user audit</p>
        </div>
