import type { Route } from "./+types/invoices.recurring";
...
interface InvoicesRecurringItem {
  id: string;
  name: string;
...
const InvoicesRecurringFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = InvoicesRecurringFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 131;
  const items: InvoicesRecurringItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `invoices-${idx + 1}`,
      name: `Invoices Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for invoices item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
export function meta() {
  return [
    { title: "Invoices Recurring | Docker Harness App" },
    { name: "description", content: "Manage invoices recurring" },
  ];
}

// ─── Component ───

export default function InvoicesRecurring({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Invoices Recurring</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track invoices recurring</p>
        </div>
