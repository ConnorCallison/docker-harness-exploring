import type { Route } from "./+types/admin.migrations";
...
const AdminMigrationsFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = AdminMigrationsFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 153;
  const items: AdminMigrationsItem[] = Array.from({ length: filters.perPage }, (_, i) => {
    const idx = (filters.page - 1) * filters.perPage + i;
    const statuses = ["active", "inactive", "pending", "archived"] as const;
    const priorities = ["low", "normal", "high", "critical"] as const;
    return {
      id: `admin-${idx + 1}`,
      name: `Admin Item ${idx + 1}`,
      status: statuses[idx % statuses.length],
      description: `Description for admin item ${idx + 1}. This is a detailed description that provides context about this particular item.`,
...
const AdminMigrationsActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = AdminMigrationsActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Admin Migrations | Docker Harness App" },
    { name: "description", content: "Manage admin migrations" },
  ];
}

// ─── Component ───

export default function AdminMigrations({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Admin Migrations</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track admin migrations</p>
        </div>
