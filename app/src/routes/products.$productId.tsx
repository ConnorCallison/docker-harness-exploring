...
const ProductDetailFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductDetailFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 154;
  const items: ProductDetailItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ProductDetailActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ProductDetailActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Product Detail | Docker Harness App" },
    { name: "description", content: "Manage product detail" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Detail</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product detail</p>
        </div>
