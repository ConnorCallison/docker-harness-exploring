...
const ProductEditFilterSchema = z.object({
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const filters = ProductEditFilterSchema.parse(Object.fromEntries(url.searchParams));

  // Simulated data generation
  const total = 219;
  const items: ProductEditItem[] = Array.from({ length: filters.perPage }, (_, i) => {
...
const ProductEditActionSchema = z.discriminatedUnion("intent", [
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const parsed = ProductEditActionSchema.safeParse(data);
...
export function meta() {
  return [
    { title: "Product Edit | Docker Harness App" },
    { name: "description", content: "Manage product edit" },
  ];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Product Edit</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track product edit</p>
        </div>
