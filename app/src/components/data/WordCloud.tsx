...
export type WordCloudVariant = "default" | "primary" | "secondary";
export type WordCloudSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const WordCloud = forwardRef<HTMLDivElement, WordCloudProps>(function WordCloud(
  ...
  const processedData = useMemo(() => {
    const raw = (props as WordCloudProps).data ?? [];
  ...
  }, [(props as WordCloudProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "WordCloud"}
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">WO</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Word Cloud</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as WordCloudProps).columns ?? []).map((col) => (
