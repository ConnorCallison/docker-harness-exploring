...
export type LazyLoadVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type LazyLoadSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const LazyLoad = forwardRef<HTMLDivElement, LazyLoadProps>(function LazyLoad(
  ...
  const processedData = useMemo(() => {
    const raw = (props as LazyLoadProps).data ?? [];
  ...
  }, [(props as LazyLoadProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "LazyLoad"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Lazy Load</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as LazyLoadProps).columns ?? []).map((col) => (
