...
export type BulkActionsVariant = "default" | "primary" | "secondary";
export type BulkActionsSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface BulkActionsProps {
  className?: string;
  children?: React.ReactNode;
  variant?: BulkActionsVariant;
  size?: BulkActionsSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const BulkActions = forwardRef<HTMLDivElement, BulkActionsProps>(function BulkActions(
  ...
  const processedData = useMemo(() => {
    const raw = (props as BulkActionsProps).data ?? [];
  ...
  }, [(props as BulkActionsProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "BulkActions"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Bulk Actions</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as BulkActionsProps).columns ?? []).map((col) => (
