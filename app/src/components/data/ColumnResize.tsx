...
export type ColumnResizeVariant = "default" | "primary" | "secondary" | "destructive";
export type ColumnResizeSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ColumnResizeProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ColumnResizeVariant;
  size?: ColumnResizeSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ColumnResize = forwardRef<HTMLDivElement, ColumnResizeProps>(function ColumnResize(
  ...
  const processedData = useMemo(() => {
    const raw = (props as ColumnResizeProps).data ?? [];
  ...
  }, [(props as ColumnResizeProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ColumnResize"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Column Resize</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as ColumnResizeProps).columns ?? []).map((col) => (
