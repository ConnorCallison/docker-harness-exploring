...
export type QueryBuilderVariant = "default" | "primary" | "secondary";
export type QueryBuilderSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface QueryBuilderProps {
  className?: string;
  children?: React.ReactNode;
  variant?: QueryBuilderVariant;
  size?: QueryBuilderSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const QueryBuilder = forwardRef<HTMLDivElement, QueryBuilderProps>(function QueryBuilder(
  ...
  const processedData = useMemo(() => {
    const raw = (props as QueryBuilderProps).data ?? [];
  ...
  }, [(props as QueryBuilderProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "QueryBuilder"}
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">QU</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Query Builder</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as QueryBuilderProps).columns ?? []).map((col) => (
