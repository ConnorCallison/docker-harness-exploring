...
export type ColumnDragVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type ColumnDragSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ColumnDragProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ColumnDragVariant;
  size?: ColumnDragSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const ColumnDrag = forwardRef<HTMLDivElement, ColumnDragProps>(function ColumnDrag(
  ...
  const processedData = useMemo(() => {
    const raw = (props as ColumnDragProps).data ?? [];
  ...
  }, [(props as ColumnDragProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ColumnDrag"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Column Drag</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as ColumnDragProps).columns ?? []).map((col) => (
