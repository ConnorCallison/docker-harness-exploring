...
export type DragSortListVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type DragSortListSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface DragSortListProps {
  className?: string;
  children?: React.ReactNode;
  variant?: DragSortListVariant;
  size?: DragSortListSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const DragSortList = forwardRef<HTMLDivElement, DragSortListProps>(function DragSortList(
  ...
  const processedData = useMemo(() => {
    const raw = (props as DragSortListProps).data ?? [];
  ...
  }, [(props as DragSortListProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "DragSortList"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Drag Sort List</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as DragSortListProps).columns ?? []).map((col) => (
