...
export type NestedTableVariant = "default" | "primary" | "secondary" | "destructive";
export type NestedTableSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface NestedTableProps {
  className?: string;
  children?: React.ReactNode;
  variant?: NestedTableVariant;
  size?: NestedTableSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const NestedTable = forwardRef<HTMLDivElement, NestedTableProps>(function NestedTable(
  ...
  const processedData = useMemo(() => {
    const raw = (props as NestedTableProps).data ?? [];
  ...
  }, [(props as NestedTableProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "NestedTable"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Nested Table</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as NestedTableProps).columns ?? []).map((col) => (
