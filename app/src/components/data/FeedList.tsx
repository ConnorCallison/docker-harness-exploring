...
export type FeedListVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type FeedListSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface FeedListProps {
  className?: string;
  children?: React.ReactNode;
  variant?: FeedListVariant;
  size?: FeedListSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const FeedList = forwardRef<HTMLDivElement, FeedListProps>(function FeedList(
  ...
  const processedData = useMemo(() => {
    const raw = (props as FeedListProps).data ?? [];
  ...
  }, [(props as FeedListProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "FeedList"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Feed List</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as FeedListProps).columns ?? []).map((col) => (
