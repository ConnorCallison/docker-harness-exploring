...
export type SavedFiltersVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type SavedFiltersSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface SavedFiltersProps {
  className?: string;
  children?: React.ReactNode;
  variant?: SavedFiltersVariant;
  size?: SavedFiltersSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const SavedFilters = forwardRef<HTMLDivElement, SavedFiltersProps>(function SavedFilters(
  ...
  const processedData = useMemo(() => {
    const raw = (props as SavedFiltersProps).data ?? [];
  ...
  }, [(props as SavedFiltersProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "SavedFilters"}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as SavedFiltersProps).columns ?? []).map((col) => (
