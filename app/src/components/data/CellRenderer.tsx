...
export type CellRendererVariant = "default" | "primary" | "secondary" | "destructive";
export type CellRendererSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface CellRendererProps {
  className?: string;
  children?: React.ReactNode;
  variant?: CellRendererVariant;
  size?: CellRendererSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const CellRenderer = forwardRef<HTMLDivElement, CellRendererProps>(function CellRenderer(
  ...
  const processedData = useMemo(() => {
    const raw = (props as CellRendererProps).data ?? [];
  ...
  }, [(props as CellRendererProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "CellRenderer"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Cell Renderer</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as CellRendererProps).columns ?? []).map((col) => (
