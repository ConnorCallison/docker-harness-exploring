...
export type PivotTableVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type PivotTableSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface PivotTableProps {
  className?: string;
  children?: React.ReactNode;
  variant?: PivotTableVariant;
  size?: PivotTableSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const PivotTable = forwardRef<HTMLDivElement, PivotTableProps>(function PivotTable(
  ...
  const processedData = useMemo(() => {
    const raw = (props as PivotTableProps).data ?? [];
  ...
  }, [(props as PivotTableProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "PivotTable"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Pivot Table</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as PivotTableProps).columns ?? []).map((col) => (
