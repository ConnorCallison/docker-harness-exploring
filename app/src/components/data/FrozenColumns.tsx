...
import type { ColumnResizeProps } from "./ColumnResize";
import type { ColumnDragProps } from "./ColumnDrag";

// ─── Types ───

export type FrozenColumnsVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type FrozenColumnsSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface FrozenColumnsProps {
  className?: string;
  children?: React.ReactNode;
  variant?: FrozenColumnsVariant;
  size?: FrozenColumnsSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const FrozenColumns = forwardRef<HTMLDivElement, FrozenColumnsProps>(function FrozenColumns(
  ...
  const processedData = useMemo(() => {
    const raw = (props as FrozenColumnsProps).data ?? [];
  ...
  }, [(props as FrozenColumnsProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "FrozenColumns"}
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">FR</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Frozen Columns</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as FrozenColumnsProps).columns ?? []).map((col) => (
