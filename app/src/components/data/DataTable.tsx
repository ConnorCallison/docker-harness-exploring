...
import type { DataExplorerProps } from "./DataExplorer";
import type { QueryBuilderProps } from "./QueryBuilder";
import type { FilterChipsProps } from "./FilterChips";

// ─── Types ───

export type DataTableVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
...
export const DataTable = forwardRef<HTMLDivElement, DataTableProps>(function DataTable(
  ...
  const processedData = useMemo(() => {
    const raw = (props as DataTableProps).data ?? [];
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Data Table</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as DataTableProps).columns ?? []).map((col) => (
