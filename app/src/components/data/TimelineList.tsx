...
import type { TableSkeletonProps } from "./TableSkeleton";
import type { EmptyStateProps } from "./EmptyState";

// ─── Types ───

export type TimelineListVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type TimelineListSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface TimelineListProps {
  className?: string;
  children?: React.ReactNode;
  variant?: TimelineListVariant;
  size?: TimelineListSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const TimelineList = forwardRef<HTMLDivElement, TimelineListProps>(function TimelineList(
  ...
  const processedData = useMemo(() => {
    const raw = (props as TimelineListProps).data ?? [];
  ...
  }, [(props as TimelineListProps).data]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "TimelineList"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Timeline List</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {((props as TimelineListProps).columns ?? []).map((col) => (
