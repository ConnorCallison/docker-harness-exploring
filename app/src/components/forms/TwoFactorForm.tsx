...
export type TwoFactorFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type TwoFactorFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface TwoFactorFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: TwoFactorFormVariant;
  size?: TwoFactorFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const TwoFactorForm = forwardRef<HTMLDivElement, TwoFactorFormProps>(function TwoFactorForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "TwoFactorForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">TW</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Two Factor Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as TwoFactorFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as TwoFactorFormProps).label}
              {(props as TwoFactorFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as TwoFactorFormProps).name}
              placeholder={(props as TwoFactorFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as TwoFactorFormProps).autoFocus}
          ...
          {(props as TwoFactorFormProps).description && (
            <p className="text-xs text-gray-500">{(props as TwoFactorFormProps).description}</p>
          )}
          {(props as TwoFactorFormProps).error && (
