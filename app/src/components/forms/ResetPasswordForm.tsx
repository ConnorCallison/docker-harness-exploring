...
export type ResetPasswordFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type ResetPasswordFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ResetPasswordFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ResetPasswordFormVariant;
  size?: ResetPasswordFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ResetPasswordForm = forwardRef<HTMLDivElement, ResetPasswordFormProps>(function ResetPasswordForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ResetPasswordForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Reset Password Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as ResetPasswordFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as ResetPasswordFormProps).label}
              {(props as ResetPasswordFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as ResetPasswordFormProps).name}
              placeholder={(props as ResetPasswordFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as ResetPasswordFormProps).autoFocus}
          ...
          {(props as ResetPasswordFormProps).description && (
            <p className="text-xs text-gray-500">{(props as ResetPasswordFormProps).description}</p>
          )}
          {(props as ResetPasswordFormProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/ResetPasswordForm</span>
