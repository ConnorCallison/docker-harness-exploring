...
export type InviteFormVariant = "default" | "primary" | "secondary" | "destructive";
export type InviteFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface InviteFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: InviteFormVariant;
  size?: InviteFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const InviteForm = forwardRef<HTMLDivElement, InviteFormProps>(function InviteForm(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Invite Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as InviteFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as InviteFormProps).label}
              {(props as InviteFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as InviteFormProps).name}
              placeholder={(props as InviteFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as InviteFormProps).autoFocus}
          ...
          {(props as InviteFormProps).description && (
            <p className="text-xs text-gray-500">{(props as InviteFormProps).description}</p>
          )}
          {(props as InviteFormProps).error && (
