...
export type TemplateEditorVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type TemplateEditorSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface TemplateEditorProps {
  className?: string;
  children?: React.ReactNode;
  variant?: TemplateEditorVariant;
  size?: TemplateEditorSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const TemplateEditor = forwardRef<HTMLDivElement, TemplateEditorProps>(function TemplateEditor(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "TemplateEditor"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">TE</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Template Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as TemplateEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as TemplateEditorProps).label}
              {(props as TemplateEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as TemplateEditorProps).name}
              placeholder={(props as TemplateEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as TemplateEditorProps).autoFocus}
          ...
          {(props as TemplateEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as TemplateEditorProps).description}</p>
          )}
          {(props as TemplateEditorProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/TemplateEditor</span>
