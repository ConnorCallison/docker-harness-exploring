...
export type ApiKeyFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type ApiKeyFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ApiKeyFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ApiKeyFormVariant;
  size?: ApiKeyFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const ApiKeyForm = forwardRef<HTMLDivElement, ApiKeyFormProps>(function ApiKeyForm(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Api Key Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as ApiKeyFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as ApiKeyFormProps).label}
              {(props as ApiKeyFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as ApiKeyFormProps).name}
              placeholder={(props as ApiKeyFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as ApiKeyFormProps).autoFocus}
          ...
          {(props as ApiKeyFormProps).description && (
            <p className="text-xs text-gray-500">{(props as ApiKeyFormProps).description}</p>
          )}
          {(props as ApiKeyFormProps).error && (
