...
export type JsonEditorVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type JsonEditorSize = "xs" | "sm" | "md" | "lg" | "xl";
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Json Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as JsonEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as JsonEditorProps).label}
              {(props as JsonEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as JsonEditorProps).name}
              placeholder={(props as JsonEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as JsonEditorProps).autoFocus}
          ...
          {(props as JsonEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as JsonEditorProps).description}</p>
          )}
          {(props as JsonEditorProps).error && (
