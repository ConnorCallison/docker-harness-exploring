...
export type RichTextEditorVariant = "default" | "primary" | "secondary";
...
export const RichTextEditor = forwardRef<HTMLDivElement, RichTextEditorProps>(function RichTextEditor(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "RichTextEditor"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Rich Text Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as RichTextEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as RichTextEditorProps).label}
              {(props as RichTextEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as RichTextEditorProps).name}
              placeholder={(props as RichTextEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as RichTextEditorProps).autoFocus}
          ...
          {(props as RichTextEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as RichTextEditorProps).description}</p>
          )}
          {(props as RichTextEditorProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/RichTextEditor</span>
