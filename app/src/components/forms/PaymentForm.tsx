...
export type PaymentFormVariant = "default" | "primary" | "secondary";
export type PaymentFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface PaymentFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: PaymentFormVariant;
  size?: PaymentFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const PaymentForm = forwardRef<HTMLDivElement, PaymentFormProps>(function PaymentForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "PaymentForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Payment Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as PaymentFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as PaymentFormProps).label}
              {(props as PaymentFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as PaymentFormProps).name}
              placeholder={(props as PaymentFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as PaymentFormProps).autoFocus}
          ...
          {(props as PaymentFormProps).description && (
            <p className="text-xs text-gray-500">{(props as PaymentFormProps).description}</p>
          )}
          {(props as PaymentFormProps).error && (
