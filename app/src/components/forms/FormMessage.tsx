...
export type FormMessageVariant = "default" | "primary" | "secondary" | "destructive";
export type FormMessageSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const FormMessage = forwardRef<HTMLDivElement, FormMessageProps>(function FormMessage(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "FormMessage"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Form Message</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as FormMessageProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as FormMessageProps).label}
              {(props as FormMessageProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as FormMessageProps).name}
              placeholder={(props as FormMessageProps).placeholder}
              disabled={disabled}
              autoFocus={(props as FormMessageProps).autoFocus}
          ...
          {(props as FormMessageProps).description && (
            <p className="text-xs text-gray-500">{(props as FormMessageProps).description}</p>
          )}
          {(props as FormMessageProps).error && (
