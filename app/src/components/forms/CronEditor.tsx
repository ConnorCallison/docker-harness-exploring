...
import type { MappingEditorProps } from "./MappingEditor";
import type { SchemaEditorProps } from "./SchemaEditor";
import type { FilterFormProps } from "./FilterForm";

// ─── Types ───

export type CronEditorVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type CronEditorSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface CronEditorProps {
  className?: string;
  children?: React.ReactNode;
  variant?: CronEditorVariant;
  size?: CronEditorSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const CronEditor = forwardRef<HTMLDivElement, CronEditorProps>(function CronEditor(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Cron Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as CronEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as CronEditorProps).label}
              {(props as CronEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as CronEditorProps).name}
              placeholder={(props as CronEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as CronEditorProps).autoFocus}
          ...
          {(props as CronEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as CronEditorProps).description}</p>
          )}
          {(props as CronEditorProps).error && (
