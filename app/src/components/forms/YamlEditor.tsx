...
export type YamlEditorVariant = "default" | "primary" | "secondary";
export type YamlEditorSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface YamlEditorProps {
  className?: string;
  children?: React.ReactNode;
  variant?: YamlEditorVariant;
  size?: YamlEditorSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const YamlEditor = forwardRef<HTMLDivElement, YamlEditorProps>(function YamlEditor(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Yaml Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as YamlEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as YamlEditorProps).label}
              {(props as YamlEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as YamlEditorProps).name}
              placeholder={(props as YamlEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as YamlEditorProps).autoFocus}
          ...
          {(props as YamlEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as YamlEditorProps).description}</p>
          )}
          {(props as YamlEditorProps).error && (
