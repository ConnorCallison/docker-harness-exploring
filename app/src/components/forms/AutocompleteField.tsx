...
export type AutocompleteFieldVariant = "default" | "primary" | "secondary";
export type AutocompleteFieldSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface AutocompleteFieldProps {
  className?: string;
  children?: React.ReactNode;
  variant?: AutocompleteFieldVariant;
...
export const AutocompleteField = forwardRef<HTMLDivElement, AutocompleteFieldProps>(function AutocompleteField(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Autocomplete Field</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as AutocompleteFieldProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as AutocompleteFieldProps).label}
              {(props as AutocompleteFieldProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as AutocompleteFieldProps).name}
              placeholder={(props as AutocompleteFieldProps).placeholder}
              disabled={disabled}
              autoFocus={(props as AutocompleteFieldProps).autoFocus}
          ...
          {(props as AutocompleteFieldProps).description && (
            <p className="text-xs text-gray-500">{(props as AutocompleteFieldProps).description}</p>
          )}
          {(props as AutocompleteFieldProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/AutocompleteField</span>
