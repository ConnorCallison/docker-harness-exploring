...
export type ExpressionInputVariant = "default" | "primary" | "secondary" | "destructive";
export type ExpressionInputSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ExpressionInputProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ExpressionInputVariant;
  size?: ExpressionInputSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ExpressionInput = forwardRef<HTMLDivElement, ExpressionInputProps>(function ExpressionInput(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ExpressionInput"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Expression Input</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as ExpressionInputProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as ExpressionInputProps).label}
              {(props as ExpressionInputProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as ExpressionInputProps).name}
              placeholder={(props as ExpressionInputProps).placeholder}
              disabled={disabled}
              autoFocus={(props as ExpressionInputProps).autoFocus}
          ...
          {(props as ExpressionInputProps).description && (
            <p className="text-xs text-gray-500">{(props as ExpressionInputProps).description}</p>
          )}
          {(props as ExpressionInputProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/ExpressionInput</span>
