...
export type FormLabelVariant = "default" | "primary" | "secondary";
...
export const FormLabel = forwardRef<HTMLDivElement, FormLabelProps>(function FormLabel(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Form Label</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as FormLabelProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as FormLabelProps).label}
              {(props as FormLabelProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as FormLabelProps).name}
              placeholder={(props as FormLabelProps).placeholder}
              disabled={disabled}
              autoFocus={(props as FormLabelProps).autoFocus}
          ...
          {(props as FormLabelProps).description && (
            <p className="text-xs text-gray-500">{(props as FormLabelProps).description}</p>
          )}
          {(props as FormLabelProps).error && (
