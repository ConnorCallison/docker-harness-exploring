...
export type WebhookFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type WebhookFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface WebhookFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: WebhookFormVariant;
  size?: WebhookFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const WebhookForm = forwardRef<HTMLDivElement, WebhookFormProps>(function WebhookForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "WebhookForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">WE</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Webhook Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as WebhookFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as WebhookFormProps).label}
              {(props as WebhookFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as WebhookFormProps).name}
              placeholder={(props as WebhookFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as WebhookFormProps).autoFocus}
          ...
          {(props as WebhookFormProps).description && (
            <p className="text-xs text-gray-500">{(props as WebhookFormProps).description}</p>
          )}
          {(props as WebhookFormProps).error && (
