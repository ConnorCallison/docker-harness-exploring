...
export type AddressFormVariant = "default" | "primary" | "secondary";
export type AddressFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface AddressFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: AddressFormVariant;
  size?: AddressFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const AddressForm = forwardRef<HTMLDivElement, AddressFormProps>(function AddressForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "AddressForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Address Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as AddressFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as AddressFormProps).label}
              {(props as AddressFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as AddressFormProps).name}
              placeholder={(props as AddressFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as AddressFormProps).autoFocus}
          ...
          {(props as AddressFormProps).description && (
            <p className="text-xs text-gray-500">{(props as AddressFormProps).description}</p>
          )}
          {(props as AddressFormProps).error && (
