...
export type RegisterFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type RegisterFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface RegisterFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: RegisterFormVariant;
  size?: RegisterFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const RegisterForm = forwardRef<HTMLDivElement, RegisterFormProps>(function RegisterForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "RegisterForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Register Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as RegisterFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as RegisterFormProps).label}
              {(props as RegisterFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as RegisterFormProps).name}
              placeholder={(props as RegisterFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as RegisterFormProps).autoFocus}
          ...
          {(props as RegisterFormProps).description && (
            <p className="text-xs text-gray-500">{(props as RegisterFormProps).description}</p>
          )}
          {(props as RegisterFormProps).error && (
