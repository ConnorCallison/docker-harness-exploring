...
export type ConditionEditorVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type ConditionEditorSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ConditionEditorProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ConditionEditorVariant;
  size?: ConditionEditorSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ConditionEditor = forwardRef<HTMLDivElement, ConditionEditorProps>(function ConditionEditor(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ConditionEditor"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Condition Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as ConditionEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as ConditionEditorProps).label}
              {(props as ConditionEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as ConditionEditorProps).name}
              placeholder={(props as ConditionEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as ConditionEditorProps).autoFocus}
          ...
          {(props as ConditionEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as ConditionEditorProps).description}</p>
          )}
          {(props as ConditionEditorProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/ConditionEditor</span>
