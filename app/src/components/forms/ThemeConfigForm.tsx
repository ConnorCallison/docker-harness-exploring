...
import type { CustomFieldFormProps } from "./CustomFieldForm";

// ─── Types ───

export type ThemeConfigFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type ThemeConfigFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ThemeConfigFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ThemeConfigFormVariant;
  size?: ThemeConfigFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ThemeConfigForm = forwardRef<HTMLDivElement, ThemeConfigFormProps>(function ThemeConfigForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ThemeConfigForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Theme Config Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as ThemeConfigFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as ThemeConfigFormProps).label}
              {(props as ThemeConfigFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as ThemeConfigFormProps).name}
              placeholder={(props as ThemeConfigFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as ThemeConfigFormProps).autoFocus}
          ...
          {(props as ThemeConfigFormProps).description && (
            <p className="text-xs text-gray-500">{(props as ThemeConfigFormProps).description}</p>
          )}
          {(props as ThemeConfigFormProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/ThemeConfigForm</span>
