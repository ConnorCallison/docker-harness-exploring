...
import type { TwoFactorFormProps } from "./TwoFactorForm";

// ─── Types ───

export type IntegrationFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type IntegrationFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface IntegrationFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: IntegrationFormVariant;
  size?: IntegrationFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const IntegrationForm = forwardRef<HTMLDivElement, IntegrationFormProps>(function IntegrationForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "IntegrationForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Integration Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as IntegrationFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as IntegrationFormProps).label}
              {(props as IntegrationFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as IntegrationFormProps).name}
              placeholder={(props as IntegrationFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as IntegrationFormProps).autoFocus}
          ...
          {(props as IntegrationFormProps).description && (
            <p className="text-xs text-gray-500">{(props as IntegrationFormProps).description}</p>
          )}
          {(props as IntegrationFormProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/IntegrationForm</span>
