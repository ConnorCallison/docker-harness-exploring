...
export type SchemaEditorVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type SchemaEditorSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface SchemaEditorProps {
  className?: string;
  children?: React.ReactNode;
  variant?: SchemaEditorVariant;
  size?: SchemaEditorSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const SchemaEditor = forwardRef<HTMLDivElement, SchemaEditorProps>(function SchemaEditor(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "SchemaEditor"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Schema Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as SchemaEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as SchemaEditorProps).label}
              {(props as SchemaEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as SchemaEditorProps).name}
              placeholder={(props as SchemaEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as SchemaEditorProps).autoFocus}
          ...
          {(props as SchemaEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as SchemaEditorProps).description}</p>
          )}
          {(props as SchemaEditorProps).error && (
