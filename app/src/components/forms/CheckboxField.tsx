...
export type CheckboxFieldVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
...
export const CheckboxField = forwardRef<HTMLDivElement, CheckboxFieldProps>(function CheckboxField(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "CheckboxField"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Checkbox Field</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as CheckboxFieldProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as CheckboxFieldProps).label}
              {(props as CheckboxFieldProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as CheckboxFieldProps).name}
              placeholder={(props as CheckboxFieldProps).placeholder}
              disabled={disabled}
              autoFocus={(props as CheckboxFieldProps).autoFocus}
          ...
          {(props as CheckboxFieldProps).description && (
            <p className="text-xs text-gray-500">{(props as CheckboxFieldProps).description}</p>
          )}
          {(props as CheckboxFieldProps).error && (
