...
export type SelectFieldVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
...
export const SelectField = forwardRef<HTMLDivElement, SelectFieldProps>(function SelectField(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "SelectField"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Select Field</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as SelectFieldProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as SelectFieldProps).label}
              {(props as SelectFieldProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as SelectFieldProps).name}
              placeholder={(props as SelectFieldProps).placeholder}
              disabled={disabled}
              autoFocus={(props as SelectFieldProps).autoFocus}
          ...
          {(props as SelectFieldProps).description && (
            <p className="text-xs text-gray-500">{(props as SelectFieldProps).description}</p>
          )}
          {(props as SelectFieldProps).error && (
