...
export type MappingEditorVariant = "default" | "primary" | "secondary" | "destructive";
export type MappingEditorSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface MappingEditorProps {
  className?: string;
  children?: React.ReactNode;
  variant?: MappingEditorVariant;
  size?: MappingEditorSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const MappingEditor = forwardRef<HTMLDivElement, MappingEditorProps>(function MappingEditor(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "MappingEditor"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Mapping Editor</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as MappingEditorProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as MappingEditorProps).label}
              {(props as MappingEditorProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as MappingEditorProps).name}
              placeholder={(props as MappingEditorProps).placeholder}
              disabled={disabled}
              autoFocus={(props as MappingEditorProps).autoFocus}
          ...
          {(props as MappingEditorProps).description && (
            <p className="text-xs text-gray-500">{(props as MappingEditorProps).description}</p>
          )}
          {(props as MappingEditorProps).error && (
