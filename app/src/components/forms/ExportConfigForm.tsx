...
export type ExportConfigFormVariant = "default" | "primary" | "secondary";
export type ExportConfigFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ExportConfigFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ExportConfigFormVariant;
  size?: ExportConfigFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ExportConfigForm = forwardRef<HTMLDivElement, ExportConfigFormProps>(function ExportConfigForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ExportConfigForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">EX</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Export Config Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as ExportConfigFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as ExportConfigFormProps).label}
              {(props as ExportConfigFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as ExportConfigFormProps).name}
              placeholder={(props as ExportConfigFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as ExportConfigFormProps).autoFocus}
          ...
          {(props as ExportConfigFormProps).description && (
            <p className="text-xs text-gray-500">{(props as ExportConfigFormProps).description}</p>
          )}
          {(props as ExportConfigFormProps).error && (
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>forms/ExportConfigForm</span>
