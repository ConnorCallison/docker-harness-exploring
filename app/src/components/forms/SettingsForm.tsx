...
import type { NotificationPrefsFormProps } from "./NotificationPrefsForm";

// ─── Types ───

export type SettingsFormVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type SettingsFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface SettingsFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: SettingsFormVariant;
  size?: SettingsFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const SettingsForm = forwardRef<HTMLDivElement, SettingsFormProps>(function SettingsForm(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "SettingsForm"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Settings Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as SettingsFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as SettingsFormProps).label}
              {(props as SettingsFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as SettingsFormProps).name}
              placeholder={(props as SettingsFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as SettingsFormProps).autoFocus}
          ...
          {(props as SettingsFormProps).description && (
            <p className="text-xs text-gray-500">{(props as SettingsFormProps).description}</p>
          )}
          {(props as SettingsFormProps).error && (
