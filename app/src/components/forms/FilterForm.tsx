...
export type FilterFormVariant = "default" | "primary" | "secondary";
export type FilterFormSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const FilterForm = forwardRef<HTMLDivElement, FilterFormProps>(function FilterForm(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Filter Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as FilterFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as FilterFormProps).label}
              {(props as FilterFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as FilterFormProps).name}
              placeholder={(props as FilterFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as FilterFormProps).autoFocus}
          ...
          {(props as FilterFormProps).description && (
            <p className="text-xs text-gray-500">{(props as FilterFormProps).description}</p>
          )}
          {(props as FilterFormProps).error && (
