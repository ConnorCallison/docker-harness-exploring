...
export type TagInputVariant = "default" | "primary" | "secondary";
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Tag Input</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as TagInputProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as TagInputProps).label}
              {(props as TagInputProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as TagInputProps).name}
              placeholder={(props as TagInputProps).placeholder}
              disabled={disabled}
              autoFocus={(props as TagInputProps).autoFocus}
          ...
          {(props as TagInputProps).description && (
            <p className="text-xs text-gray-500">{(props as TagInputProps).description}</p>
          )}
          {(props as TagInputProps).error && (
