...
export type DateFieldVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Date Field</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as DateFieldProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as DateFieldProps).label}
              {(props as DateFieldProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as DateFieldProps).name}
              placeholder={(props as DateFieldProps).placeholder}
              disabled={disabled}
              autoFocus={(props as DateFieldProps).autoFocus}
          ...
          {(props as DateFieldProps).description && (
            <p className="text-xs text-gray-500">{(props as DateFieldProps).description}</p>
          )}
          {(props as DateFieldProps).error && (
