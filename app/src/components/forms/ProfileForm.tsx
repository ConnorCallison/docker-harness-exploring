...
export type ProfileFormVariant = "default" | "primary" | "secondary";
export type ProfileFormSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ProfileFormProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ProfileFormVariant;
  size?: ProfileFormSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ProfileForm = forwardRef<HTMLDivElement, ProfileFormProps>(function ProfileForm(
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Profile Form</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as ProfileFormProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as ProfileFormProps).label}
              {(props as ProfileFormProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as ProfileFormProps).name}
              placeholder={(props as ProfileFormProps).placeholder}
              disabled={disabled}
              autoFocus={(props as ProfileFormProps).autoFocus}
          ...
          {(props as ProfileFormProps).description && (
            <p className="text-xs text-gray-500">{(props as ProfileFormProps).description}</p>
          )}
          {(props as ProfileFormProps).error && (
