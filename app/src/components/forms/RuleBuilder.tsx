...
export type RuleBuilderVariant = "default" | "primary" | "secondary" | "destructive";
export type RuleBuilderSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface RuleBuilderProps {
  className?: string;
  children?: React.ReactNode;
  variant?: RuleBuilderVariant;
  size?: RuleBuilderSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const RuleBuilder = forwardRef<HTMLDivElement, RuleBuilderProps>(function RuleBuilder(
  ...
  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "RuleBuilder"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">RU</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Rule Builder</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as RuleBuilderProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as RuleBuilderProps).label}
              {(props as RuleBuilderProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as RuleBuilderProps).name}
              placeholder={(props as RuleBuilderProps).placeholder}
              disabled={disabled}
              autoFocus={(props as RuleBuilderProps).autoFocus}
          ...
          {(props as RuleBuilderProps).description && (
            <p className="text-xs text-gray-500">{(props as RuleBuilderProps).description}</p>
          )}
          {(props as RuleBuilderProps).error && (
