...
export type PhoneInputVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type PhoneInputSize = "xs" | "sm" | "md" | "lg" | "xl";
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">PH</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Phone Input</h3>
      ...
      <div className="p-4 space-y-4">
        <div className="space-y-3">
          {(props as PhoneInputProps).label && (
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {(props as PhoneInputProps).label}
              {(props as PhoneInputProps).required && <span className="text-red-500 ml-1">*</span>}
            </label>
          )}
          <div className="relative">
            <input
              type="text"
              name={(props as PhoneInputProps).name}
              placeholder={(props as PhoneInputProps).placeholder}
              disabled={disabled}
              autoFocus={(props as PhoneInputProps).autoFocus}
          ...
          {(props as PhoneInputProps).description && (
            <p className="text-xs text-gray-500">{(props as PhoneInputProps).description}</p>
          )}
          {(props as PhoneInputProps).error && (
