...
import type { BoxPlotChartProps } from "./BoxPlotChart";

// ─── Types ───

export type BulletChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type BulletChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface BulletChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: BulletChartVariant;
...
export const BulletChart = forwardRef<HTMLDivElement, BulletChartProps>(function BulletChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as BulletChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as BulletChartProps).color ?? "#3b82f6",
    }));
  }, [(props as BulletChartProps).data, (props as BulletChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "BulletChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Bullet Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as BulletChartProps).height ?? 300 }}>
