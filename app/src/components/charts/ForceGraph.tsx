...
export type ForceGraphVariant = "default" | "primary" | "secondary";
export type ForceGraphSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const ForceGraph = forwardRef<HTMLDivElement, ForceGraphProps>(function ForceGraph(
  ...
  const chartData = useMemo(() => {
    const raw = (props as ForceGraphProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as ForceGraphProps).color ?? "#3b82f6",
    }));
  }, [(props as ForceGraphProps).data, (props as ForceGraphProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ForceGraph"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Force Graph</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as ForceGraphProps).height ?? 300 }}>
