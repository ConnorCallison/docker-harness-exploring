...
export type StreamGraphVariant = "default" | "primary" | "secondary";
export type StreamGraphSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface StreamGraphProps {
  className?: string;
  children?: React.ReactNode;
  variant?: StreamGraphVariant;
...
export const StreamGraph = forwardRef<HTMLDivElement, StreamGraphProps>(function StreamGraph(
  ...
  const chartData = useMemo(() => {
    const raw = (props as StreamGraphProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as StreamGraphProps).color ?? "#3b82f6",
    }));
  }, [(props as StreamGraphProps).data, (props as StreamGraphProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "StreamGraph"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Stream Graph</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as StreamGraphProps).height ?? 300 }}>
