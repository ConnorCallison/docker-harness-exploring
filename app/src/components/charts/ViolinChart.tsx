...
export type ViolinChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type ViolinChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ViolinChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ViolinChartVariant;
  size?: ViolinChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ViolinChart = forwardRef<HTMLDivElement, ViolinChartProps>(function ViolinChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as ViolinChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as ViolinChartProps).color ?? "#3b82f6",
    }));
  }, [(props as ViolinChartProps).data, (props as ViolinChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ViolinChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">VI</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Violin Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as ViolinChartProps).height ?? 300 }}>
