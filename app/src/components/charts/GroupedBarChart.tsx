...
import type { DensityPlotProps } from "./DensityPlot";

// ─── Types ───

export type GroupedBarChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type GroupedBarChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface GroupedBarChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: GroupedBarChartVariant;
  size?: GroupedBarChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const GroupedBarChart = forwardRef<HTMLDivElement, GroupedBarChartProps>(function GroupedBarChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as GroupedBarChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as GroupedBarChartProps).color ?? "#3b82f6",
    }));
  }, [(props as GroupedBarChartProps).data, (props as GroupedBarChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "GroupedBarChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">GR</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Grouped Bar Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as GroupedBarChartProps).height ?? 300 }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>charts/GroupedBarChart</span>
