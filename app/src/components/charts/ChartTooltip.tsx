...
export type ChartTooltipVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type ChartTooltipSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const ChartTooltip = forwardRef<HTMLDivElement, ChartTooltipProps>(function ChartTooltip(
  ...
  const chartData = useMemo(() => {
    const raw = (props as ChartTooltipProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as ChartTooltipProps).color ?? "#3b82f6",
    }));
  }, [(props as ChartTooltipProps).data, (props as ChartTooltipProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ChartTooltip"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Chart Tooltip</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as ChartTooltipProps).height ?? 300 }}>
