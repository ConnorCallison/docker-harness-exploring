...
export type ComposedChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type ComposedChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface ComposedChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: ComposedChartVariant;
  size?: ComposedChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const ComposedChart = forwardRef<HTMLDivElement, ComposedChartProps>(function ComposedChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as ComposedChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as ComposedChartProps).color ?? "#3b82f6",
    }));
  }, [(props as ComposedChartProps).data, (props as ComposedChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ComposedChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Composed Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as ComposedChartProps).height ?? 300 }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>charts/ComposedChart</span>
