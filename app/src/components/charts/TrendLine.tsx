...
export type TrendLineVariant = "default" | "primary" | "secondary" | "destructive";
export type TrendLineSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface TrendLineProps {
  className?: string;
  children?: React.ReactNode;
  variant?: TrendLineVariant;
  size?: TrendLineSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const TrendLine = forwardRef<HTMLDivElement, TrendLineProps>(function TrendLine(
  ...
  const chartData = useMemo(() => {
    const raw = (props as TrendLineProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as TrendLineProps).color ?? "#3b82f6",
    }));
  }, [(props as TrendLineProps).data, (props as TrendLineProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "TrendLine"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Trend Line</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as TrendLineProps).height ?? 300 }}>
