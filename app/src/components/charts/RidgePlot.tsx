...
export type RidgePlotVariant = "default" | "primary" | "secondary" | "destructive";
export type RidgePlotSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const RidgePlot = forwardRef<HTMLDivElement, RidgePlotProps>(function RidgePlot(
  ...
  const chartData = useMemo(() => {
    const raw = (props as RidgePlotProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as RidgePlotProps).color ?? "#3b82f6",
    }));
  }, [(props as RidgePlotProps).data, (props as RidgePlotProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "RidgePlot"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">RI</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Ridge Plot</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as RidgePlotProps).height ?? 300 }}>
