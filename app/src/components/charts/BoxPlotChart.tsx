...
export type BoxPlotChartVariant = "default" | "primary" | "secondary";
export type BoxPlotChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface BoxPlotChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: BoxPlotChartVariant;
  size?: BoxPlotChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const BoxPlotChart = forwardRef<HTMLDivElement, BoxPlotChartProps>(function BoxPlotChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as BoxPlotChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as BoxPlotChartProps).color ?? "#3b82f6",
    }));
  }, [(props as BoxPlotChartProps).data, (props as BoxPlotChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "BoxPlotChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Box Plot Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as BoxPlotChartProps).height ?? 300 }}>
