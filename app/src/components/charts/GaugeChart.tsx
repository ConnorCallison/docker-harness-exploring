...
export type GaugeChartVariant = "default" | "primary" | "secondary";
export type GaugeChartSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const GaugeChart = forwardRef<HTMLDivElement, GaugeChartProps>(function GaugeChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as GaugeChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as GaugeChartProps).color ?? "#3b82f6",
    }));
  }, [(props as GaugeChartProps).data, (props as GaugeChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "GaugeChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Gauge Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as GaugeChartProps).height ?? 300 }}>
