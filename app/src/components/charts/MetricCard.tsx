...
import type { RidgePlotProps } from "./RidgePlot";

// ─── Types ───

export type MetricCardVariant = "default" | "primary" | "secondary" | "destructive";
export type MetricCardSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const MetricCard = forwardRef<HTMLDivElement, MetricCardProps>(function MetricCard(
  ...
  const chartData = useMemo(() => {
    const raw = (props as MetricCardProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as MetricCardProps).color ?? "#3b82f6",
    }));
  }, [(props as MetricCardProps).data, (props as MetricCardProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "MetricCard"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Metric Card</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as MetricCardProps).height ?? 300 }}>
