...
export type BubbleChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type BubbleChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface BubbleChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: BubbleChartVariant;
  size?: BubbleChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const BubbleChart = forwardRef<HTMLDivElement, BubbleChartProps>(function BubbleChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as BubbleChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as BubbleChartProps).color ?? "#3b82f6",
    }));
  }, [(props as BubbleChartProps).data, (props as BubbleChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "BubbleChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Bubble Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as BubbleChartProps).height ?? 300 }}>
