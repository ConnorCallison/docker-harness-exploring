...
export type DensityPlotVariant = "default" | "primary" | "secondary";
export type DensityPlotSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface DensityPlotProps {
  className?: string;
  children?: React.ReactNode;
  variant?: DensityPlotVariant;
  size?: DensityPlotSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const DensityPlot = forwardRef<HTMLDivElement, DensityPlotProps>(function DensityPlot(
  ...
  const chartData = useMemo(() => {
    const raw = (props as DensityPlotProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as DensityPlotProps).color ?? "#3b82f6",
    }));
  }, [(props as DensityPlotProps).data, (props as DensityPlotProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "DensityPlot"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Density Plot</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as DensityPlotProps).height ?? 300 }}>
