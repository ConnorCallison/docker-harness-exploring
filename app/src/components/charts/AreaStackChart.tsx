...
export type AreaStackChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type AreaStackChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface AreaStackChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: AreaStackChartVariant;
  size?: AreaStackChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const AreaStackChart = forwardRef<HTMLDivElement, AreaStackChartProps>(function AreaStackChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as AreaStackChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as AreaStackChartProps).color ?? "#3b82f6",
    }));
  }, [(props as AreaStackChartProps).data, (props as AreaStackChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "AreaStackChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Area Stack Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as AreaStackChartProps).height ?? 300 }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>charts/AreaStackChart</span>
