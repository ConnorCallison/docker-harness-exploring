...
export type ChartLegendVariant = "default" | "primary" | "secondary" | "destructive";
export type ChartLegendSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const ChartLegend = forwardRef<HTMLDivElement, ChartLegendProps>(function ChartLegend(
  ...
  const chartData = useMemo(() => {
    const raw = (props as ChartLegendProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as ChartLegendProps).color ?? "#3b82f6",
    }));
  }, [(props as ChartLegendProps).data, (props as ChartLegendProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ChartLegend"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Chart Legend</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as ChartLegendProps).height ?? 300 }}>
