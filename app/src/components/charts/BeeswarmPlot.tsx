...
export type BeeswarmPlotVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost";
export type BeeswarmPlotSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface BeeswarmPlotProps {
  className?: string;
  children?: React.ReactNode;
  variant?: BeeswarmPlotVariant;
  size?: BeeswarmPlotSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const BeeswarmPlot = forwardRef<HTMLDivElement, BeeswarmPlotProps>(function BeeswarmPlot(
  ...
  const chartData = useMemo(() => {
    const raw = (props as BeeswarmPlotProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as BeeswarmPlotProps).color ?? "#3b82f6",
    }));
  }, [(props as BeeswarmPlotProps).data, (props as BeeswarmPlotProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "BeeswarmPlot"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">BE</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Beeswarm Plot</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as BeeswarmPlotProps).height ?? 300 }}>
