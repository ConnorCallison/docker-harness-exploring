...
import type { GroupedBarChartProps } from "./GroupedBarChart";
import type { AreaStackChartProps } from "./AreaStackChart";
import type { MultiAxisChartProps } from "./MultiAxisChart";

// ─── Types ───

export type CandlestickChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type CandlestickChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface CandlestickChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: CandlestickChartVariant;
  size?: CandlestickChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const CandlestickChart = forwardRef<HTMLDivElement, CandlestickChartProps>(function CandlestickChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as CandlestickChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as CandlestickChartProps).color ?? "#3b82f6",
    }));
  }, [(props as CandlestickChartProps).data, (props as CandlestickChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "CandlestickChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Candlestick Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as CandlestickChartProps).height ?? 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <div className="flex items-center justify-center h-full bg-gray-50 dark:bg-gray-800/50 rounded-lg">
              <span className="text-gray-400">Chart: {chartData.length} data points</span>
            </div>
          </ResponsiveContainer>
        </div>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>charts/CandlestickChart</span>
