...
export type SankeyChartVariant = "default" | "primary" | "secondary" | "destructive";
export type SankeyChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface SankeyChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: SankeyChartVariant;
  size?: SankeyChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const SankeyChart = forwardRef<HTMLDivElement, SankeyChartProps>(function SankeyChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as SankeyChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as SankeyChartProps).color ?? "#3b82f6",
    }));
  }, [(props as SankeyChartProps).data, (props as SankeyChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "SankeyChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Sankey Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as SankeyChartProps).height ?? 300 }}>
