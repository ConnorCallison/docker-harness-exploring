...
import type { ViolinChartProps } from "./ViolinChart";

// ─── Types ───

export type StatCardVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type StatCardSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const StatCard = forwardRef<HTMLDivElement, StatCardProps>(function StatCard(
  ...
  const chartData = useMemo(() => {
    const raw = (props as StatCardProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as StatCardProps).color ?? "#3b82f6",
    }));
  }, [(props as StatCardProps).data, (props as StatCardProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "StatCard"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Stat Card</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as StatCardProps).height ?? 300 }}>
