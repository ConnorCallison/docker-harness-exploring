...
export type ChordDiagramVariant = "default" | "primary" | "secondary";
export type ChordDiagramSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const ChordDiagram = forwardRef<HTMLDivElement, ChordDiagramProps>(function ChordDiagram(
  ...
  const chartData = useMemo(() => {
    const raw = (props as ChordDiagramProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as ChordDiagramProps).color ?? "#3b82f6",
    }));
  }, [(props as ChordDiagramProps).data, (props as ChordDiagramProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "ChordDiagram"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Chord Diagram</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as ChordDiagramProps).height ?? 300 }}>
