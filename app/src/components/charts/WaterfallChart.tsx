...
export type WaterfallChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type WaterfallChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface WaterfallChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: WaterfallChartVariant;
  size?: WaterfallChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const WaterfallChart = forwardRef<HTMLDivElement, WaterfallChartProps>(function WaterfallChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as WaterfallChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as WaterfallChartProps).color ?? "#3b82f6",
    }));
  }, [(props as WaterfallChartProps).data, (props as WaterfallChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "WaterfallChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Waterfall Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as WaterfallChartProps).height ?? 300 }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
        <span>charts/WaterfallChart</span>
