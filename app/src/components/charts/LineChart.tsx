...
export type LineChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline";
export type LineChartSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const LineChart = forwardRef<HTMLDivElement, LineChartProps>(function LineChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as LineChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as LineChartProps).color ?? "#3b82f6",
    }));
  }, [(props as LineChartProps).data, (props as LineChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "LineChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Line Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as LineChartProps).height ?? 300 }}>
