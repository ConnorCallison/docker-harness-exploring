...
export type TreemapChartVariant = "default" | "primary" | "secondary" | "destructive" | "outline" | "ghost" | "link";
export type TreemapChartSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface TreemapChartProps {
  className?: string;
  children?: React.ReactNode;
  variant?: TreemapChartVariant;
  size?: TreemapChartSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
  testId?: string;
...
export const TreemapChart = forwardRef<HTMLDivElement, TreemapChartProps>(function TreemapChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as TreemapChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as TreemapChartProps).color ?? "#3b82f6",
    }));
  }, [(props as TreemapChartProps).data, (props as TreemapChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "TreemapChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Treemap Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as TreemapChartProps).height ?? 300 }}>
