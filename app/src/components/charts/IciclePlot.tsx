...
export type IciclePlotVariant = "default" | "primary" | "secondary";
export type IciclePlotSize = "xs" | "sm" | "md" | "lg" | "xl";

export interface IciclePlotProps {
  className?: string;
  children?: React.ReactNode;
  variant?: IciclePlotVariant;
  size?: IciclePlotSize;
  disabled?: boolean;
  loading?: boolean;
  id?: string;
...
export const IciclePlot = forwardRef<HTMLDivElement, IciclePlotProps>(function IciclePlot(
  ...
  const chartData = useMemo(() => {
    const raw = (props as IciclePlotProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as IciclePlotProps).color ?? "#3b82f6",
    }));
  }, [(props as IciclePlotProps).data, (props as IciclePlotProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "IciclePlot"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Icicle Plot</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as IciclePlotProps).height ?? 300 }}>
