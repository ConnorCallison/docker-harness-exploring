...
export type HeatmapChartVariant = "default" | "primary" | "secondary";
export type HeatmapChartSize = "xs" | "sm" | "md" | "lg" | "xl";
...
export const HeatmapChart = forwardRef<HTMLDivElement, HeatmapChartProps>(function HeatmapChart(
  ...
  const chartData = useMemo(() => {
    const raw = (props as HeatmapChartProps).data ?? [];
    return raw.map((d, i) => ({
      ...d,
      _color: d.category ? `hsl(${(i * 360) / raw.length}, 70%, 50%)` : (props as HeatmapChartProps).color ?? "#3b82f6",
    }));
  }, [(props as HeatmapChartProps).data, (props as HeatmapChartProps).color]);

  return (
    <div
      ref={ref}
      id={id}
      data-testid={testId ?? "HeatmapChart"}
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-white font-semibold text-sm">HE</span>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Heatmap Chart</h3>
      ...
      <div className="p-4 space-y-4">
        <div style={{ height: (props as HeatmapChartProps).height ?? 300 }}>
